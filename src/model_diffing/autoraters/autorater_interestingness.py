...
_prompts = load_prompts("autoraters/interestingness")
...
def aggregate_responses(judge_responses: list[dict]) -> dict:
    ...
    signal_keys = ["impact", "novelty", "specificity", "actionability"]
    avg_signals = {}
...
def rate_hypotheses(
    ...
    """Rate hypotheses for interestingness using multiple LLM judges.
    ...
    def adjust_hypothesis_format(hypothesis: str) -> str:
        ...
        elif hypothesis.startswith("Hypothesis: The model"):
    ...
    if output_file is None:
        output_file = hypotheses_file.replace(".json", "_autorater_responses.json")
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(output_data, f, indent=2)
    print(f"\nWrote autorater responses to {output_file}")
